...
_xonsh_mise_activate = str(XSH.env.get("XONSH_MISE_ACTIVATE", XSH.env.get("BASH_MISE_ACTIVATE", "shims"))).lower()
...
if _xonsh_mise_activate in ("full", "1", "yes", "true"):
    import shutil
    import subprocess
    ...
    if _xonsh_mise:
        _xonsh_mise_init = subprocess.run(
            [_xonsh_mise, "activate", "xonsh"],
            capture_output=True,
            text=True,
            check=False,
            env=XSH.env.detype(),
