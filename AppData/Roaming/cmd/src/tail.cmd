...
if %file_count%==0 (
    ...
    echo Try 'tail --help' for more information.
    exit /b 1
)
...
:: Function to output last N lines of a file
:tail_file
set "filepath=%~1"
set "lines=%~2"
...
if not exist "%filepath%" (
    echo tail: cannot open '%filepath%' for reading: No such file or directory
    exit /b 1
)

:: Check if it's a directory
if exist "%filepath%\" (
    echo tail: error reading '%filepath%': Is a directory
    exit /b 1
)
...
:: Output lines starting from start_line
set "current_line=0"
for /f "usebackq delims=" %%L in ("%filepath%") do (
    set /a current_line+=1
    if !current_line! GEQ %start_line% echo %%L
)
...
echo.
echo Options:
echo   -n, --lines=NUM      Print last NUM lines instead of last 10
echo   -NUM                 Same as --lines=NUM
echo   -h, --help           Display this help and exit
echo.
echo Examples:
echo   tail file1               Display last 10 lines of file1
echo   tail -n 20 file1         Display last 20 lines of file1
echo   tail -5 file1            Display last 5 lines of file1
echo   tail file1 file2         Display last 10 lines of file1 and file2
exit /b 0
